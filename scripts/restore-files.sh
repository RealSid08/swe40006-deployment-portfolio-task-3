...
cleanup() {
  result=$?
  if [[ $result -ne 0 && -d "$work/original-html" ]]; then
    systemctl stop httpd || true
    [[ ! -d /var/www/html ]] || mv /var/www/html "$work/failed-html"
    mv "$work/original-html" /var/www/html
    systemctl restart php-fpm httpd
    echo 'Restore failed; reinstated the original files.' >&2
  fi
  rm -rf "$work"
}
trap cleanup EXIT
...
if ! curl --fail --retry 10 --retry-delay 2 http://127.0.0.1/health.php; then
  echo 'Restore health validation failed.' >&2
  exit 1
fi
